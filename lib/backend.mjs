...
export function createHandler({ store, account }) {
  ...
  return async function handler(req) {
    try {
      ...
      if (path === '/api/event-info' || path === '/api/event-info/delete') {
        const saved = await store.get('event-info', { type: 'json' }) || {};
        const events = saved.events || ((saved.image || saved.description) ? [{ ...saved, id: 'legacy', title: 'Upcoming Event' }] : []);
        const current = events.find(event => event.id === data.id);
        if (data.id && !current) fail(404, 'Event not found. Refresh the page.');
        if (path.endsWith('/delete')) {
          if (!current) fail(404, 'Event not found.');
          const next = { events: events.filter(event => event.id !== data.id) };
          await store.setJSON('event-info', next); return json(next);
        }
        if (typeof data.title !== 'string' || !data.title.trim() || data.title.length > 120) fail(400, 'Enter an event title under 120 characters.');
        ...
        if (!image) fail(400, 'Choose one picture for this event.');
        const event = { id: current?.id || randomBytes(16).toString('hex'), title: data.title.trim(), image, description: data.description };
        const next = { events: current ? events.map(item => item.id === current.id ? event : item) : [...events, event] };
        await store.setJSON('event-info', next); return json(next);
      }
